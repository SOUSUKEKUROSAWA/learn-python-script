...
- `pip install pywhatkit`
  - 依存関係が多いから仮想環境でインストールした方がいい
    - 仮想環境への移行はプロジェクトの途中からでも行える
- Pythonの仮想環境はなぜ必要か
  - プロジェクト間の依存関係の衝突を防ぐ
  - プロジェクトの再現性を保つ
  - ※仮想環境（myenv）ディレクトリは，プロジェクトのPythonパッケージの依存関係を隔離するためのものであり、プロジェクトのソースコード自体を含むものではない
    - したがって、Pythonスクリプトやその他のプロジェクトファイルをmyenvディレクトリに移動させる必要はない
- Pythonの仮想環境の作り方（プロジェクト途中での移行も可能）
  - 仮想環境の作成
    - `python -m venv <venv name>`
      - ここで作成される`<venv name>`ディレクトリはGitの追跡対象に含めないべき
  - 仮想環境のアクティベート
    - `source <venv name>/Scripts/activate`
      - OSによっては`<venv name>/bin/activate`にアクティベート用のスクリプトが用意されている場合もある
  - 要件ファイルの作成
    - `pip freeze > requirements.txt`
      - これにより，現時点でインストールされているパッケージとそのバージョンがrequirements.txtに記録される
  - ディアクティベート
    - `deactivate`
- WhatsAppというアプリがインストールされている必要がある
  - チャットサービス